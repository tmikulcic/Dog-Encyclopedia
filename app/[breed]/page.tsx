...
const BreedPage = async ({ params }: { params: BreedPageProps }) => {
  ...
  const breedOverview = dogsData.find((dog) => dog.breed.toLowerCase() === breed);

  return (
    <MainContainer>
      <div className={styles.imageName}>
        <Image className={styles.breedImage} src={breedData[0].image_link} alt={breedData[0].name} width={300} height={300} priority />
        <div>
          <h2>{breedData[0].name}</h2>
          <p>{breedOverview?.overview}</p>
        </div>
      </div>
